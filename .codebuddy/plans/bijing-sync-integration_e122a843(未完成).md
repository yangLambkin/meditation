...
overview: 新增独立云函数 bijingSync，通过"绑定必经之路学号 + 每日凌晨4点定时同步绑定用户前一天静坐时长到外部系统"打通数据，含手动兜底、去重与绑定后昵称覆盖。仅用云端 meditation_records 聚合求时长，测试环境先闭环。
todos:
  - id: cf-bijing-sync
    content: 新建 bijingSync 云函数：index.js/package.json/config.json，含绑定校验(昵称覆盖)/聚合时长/去重同步/定时触发
    status: pending
  - id: front-me-binding
  ...
  - id: front-bijing-api
    ...
    dependencies:
      - cf-bijing-sync
  - id: bijing-guide-doc
    content: 编写必经之路接入指引文档：测试验证步骤(含昵称覆盖)/环境变量/生产切换说明
    status: pending
    dependencies:
      - cf-bijing-sync
  - id: deploy-verify
...
**核心思路**：新建独立云函数 `bijingSync`，入口按 `event.type` 分发：`bindStudentNumber`（绑定校验 + 昵称覆盖）、`syncUserYesterday`（单个用户同步昨天，供定时与手动复用）、`cronSyncAll`（定时扫描全部绑定用户）、`manualSyncPending`（手动补同步当前用户所有未同步历史日）。所有对外调用在云函数内完成，固定 Token 与环境变量不暴露前端。
...
4. 昵称覆盖位置与时机：绑定校验 `GET /users/{sn}` 成功后，若 `data.nickname` 非空则写 `users.nickName`（复用现有 `users` 集合结构，由 bijingSync 直接 update 该文档）。前端在 `bijingApi.bindBijing` 成功回调中：① 调 `meditationManager.updateUserProfile` 或直接依赖云端已写；② `wx.setStorageSync('userNickname', 对方昵称)`；③ `this.setData({userNickname})`，保证 me 页即时生效。昵称为空时跳过覆盖，保留原昵称。
5. 去重标记 `bijingSyncedDates`：`syncDate(openid, userDoc, dateStr)` 先查 `users.bijingSyncedDates[dateStr]`，存在则返回 `{skipped:true}`；调对端成功后才写入标记。定时只同步昨天、手动同步所有未标记日，天然不重复；即便标记写入失败，对方覆盖语义也保证数据正确，仅会重试一次。
6. 安全：`BIJING_API_BASE`、`BIJING_ACCESS_TOKEN` 仅存云函数环境变量；前端只传学号与操作类型。
7. 失败处理：单次同步失败记日志、不写标记（可重试）；昵称覆盖失败不阻断绑定；cron 扫描不因单个用户失败而中断（try/catch 包裹单用户）；不影响用户打卡主链路（不改动 `recordMeditation`/`checkin.js`）。
...
- `bijingSync/index.js`：`getBusinessDate`（复用 meditationManager 的 +8h 技巧）、`getApiBase()`/`getToken()`（读 env）、`aggregateDuration(openid, dateStr)`、`callBijingGet(path)`/`callBijingPost(path, body)`（axios + X-Access-Token）、`syncDate(openid, userDoc, dateStr)`（查标记→聚合→POST→写标记）、`bindStudentNumber(openid, sn)`（GET 校验→写 users 含昵称覆盖→返回 nickname）、`cronSyncAll()`（扫绑定用户→各 syncDate 昨天）、`manualSyncPending(openid)`（扫未标记日→syncDate）、`main` 分发（含 `event.type==='timer'` 或 cron 触发走 `cronSyncAll`）。
- `bijingSync/package.json`：`wx-server-sdk`、`axios`。
- `bijingSync/config.json`：`triggers` 数组含 timer `0 0 4 * * * *`。
- 前端 `me` 页：`me.js` 增加 `bijingStudentNumber`/`bijingBound` 读取与展示；新增绑定表单（学号输入）调 `callFunction(bijingSync, bindStudentNumber)`；绑定成功回调中更新 `userNickname` 缓存与 `setData`；新增"立即同步"按钮调 `manualSyncPending` 并 toast 结果。
- `bijingApi.js`：封装 `bindBijing(sn)`、`syncBijingPending()` 两个 callFunction 方法，保持与现有 `cloudApi` 一致的 `{success,data,error}` 信封。
...
cloudfunctions/
└── bijingSync/
    ├── index.js      # [NEW] 绑定校验(含昵称覆盖)/聚合时长/去重同步/定时全量/手动补同步；东八区昨天计算；env 读 Token 与 Base
...
```js
// cloudfunctions/bijingSync/index.js 核心片段（无 emoji）
