...
- **必经同步**：每日执行记录仅查询最近 7 个已结束静坐日（含第 7 天），按静坐日期降序、同日执行时间降序排列，单次最多 50 条；按所选日期查看也受此范围限制。展示成功/失败/跳过人数、开始/结束时间与耗时，支持分页查看逐学号错误、一键同步/续跑。补同步仍可手动选择最近 30 个已结束静坐日，超出历史展示范围的进行中任务通过独立状态继续处理。
...
更新每日执行记录的最近七天范围与排序时：先在 `bijing_sync_runs` 建立 `recordDate` 降序、`_id` 降序复合索引并确认可用，再部署 `bijingSync` 云函数，最后发布小程序。进行中任务查询继续使用既有的 `status` 升序、`startedAt` 升序索引；历史记录无需删除。
...
6. **检查/建立查询索引**（不要求唯一，`_id` 自带唯一索引）：
   ...
   | `bijing_sync_runs` | `recordDate` 降序、`_id` 降序（最近七天历史查询；新增此索引，原升序索引可保留） |
