...
test('check-in days turn over at Beijing 04:00, including month, year and leap-day boundaries', () => {
  for (const [timestamp, expected] of [
    ['2026-09-19T03:59:59.999+08:00', '2026-09-18'],
    ['2026-09-19T04:00:00+08:00', '2026-09-19'],
    ['2026-10-01T03:59:00+08:00', '2026-09-30'],
    ['2026-01-01T03:59:00+08:00', '2025-12-31'],
    ['2026-01-01T04:00:00+08:00', '2026-01-01'],
    ['2024-03-01T03:59:00+08:00', '2024-02-29']
  ]) {
...
test('day grouping is independent of the host timezone', () => {
  ...
  const script = `
    ...
    const times = ['2026-09-18T19:59:59.999Z', '2026-09-18T20:00:00Z'];
  ...
  for (const timezone of ['UTC', 'Asia/Shanghai', 'America/Los_Angeles', 'Pacific/Auckland']) {
    ...
    assert.deepEqual(JSON.parse(result.stdout), {
      ...
      records: [
        { dayDate: '2026-09-19', timeLabel: '04:00' },
        { dayDate: '2026-09-18', timeLabel: '次日 03:59' }
      ]
    }, timezone);
  }
});
...
test('forms retain actual calendar dates and times before the check-in day boundary', () => {
  const timestamp = Date.parse('2026-01-01T03:59:00+08:00');
  assert.deepEqual(getDateTime(timestamp), { date: '2026-01-01', time: '03:59' });
  assert.equal(parseDateTime('2026-01-01', '03:59'), timestamp);
  assert.equal(getCheckinDay(timestamp), '2025-12-31');
  assert.ok(Number.isNaN(parseDateTime('2026-02-30', '03:59')));
});

test('records use timestamps for grouping while retaining stored dates and record identities', () => {
  const timestamp = Date.parse('2026-09-19T02:00:00+08:00');
  ...
  assert.deepEqual(records[0], {
    ...
    date: '2026-09-19', dayDate: '2026-09-18', time: '02:00', timeLabel: '次日 02:00',
...
test('mixed timestamp formats sort by logical day and instant regardless of stored buckets', () => {
  ...
  const numericString = String(Date.parse('2026-09-19T03:30:00+08:00'));
  const records = buildCheckinRecords({ dailyRecords: {
    '2026-09-20': { records: [{ _id: 'late-evening', timestamp: numeric }] },
    '2026-09-19': { records: [{ _id: 'early-morning', timestamp: numericString }] },
    '2026-09-17': { records: [{ _id: 'new-day', timestamp: '2026-09-19T04:00:00+08:00' }] }
  } });
  ...
  assert.deepEqual(records.map(record => record.timeLabel), ['04:00', '次日 03:30', '23:00']);
...
test('default groups contain the current logical day and preceding two consecutive days', () => {
  const records = recordsFor([
    ...
    '2026-01-01T03:59:00+08:00',
    '2026-01-01T04:00:00+08:00'
  ]);
  const beforeBoundary = buildCheckinGroups(records, { now: '2026-01-01T03:59:59+08:00' });
  ...
  const afterBoundary = buildCheckinGroups(records, { now: '2026-01-01T04:00:00+08:00' });
...
test('complete history groups by the month of each 04:00 day, newest months and days first', () => {
  const records = recordsFor([
    '2025-12-31T22:00:00+08:00',
    '2026-01-01T03:59:59+08:00',
    '2026-01-01T04:00:00+08:00',
    '2026-02-01T03:00:00+08:00',
    '2026-02-01T04:00:00+08:00'
  ]);
  ...
  assert.equal(months[1].days[0].records[0].timeLabel, '次日 03:00');
