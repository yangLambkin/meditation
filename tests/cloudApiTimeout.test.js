...
test('a cloud request with no callback rejects after five seconds and ignores late callbacks', async () => {
  const app = harness();
...
for (const [name, type] of [
  ...
  test(`unrelated cloud operations keep their original timeout behavior (${name}/${type})`, async () => {
    ...
    assert.equal(app.timers.size, 0, 'only check-in synchronization operations get a five-second deadline');
    const response = { result: { success: true } };
    app.requests[0].success(response);
    assert.strictEqual(await pending, response);
  });
...
test('a content-check timeout keeps the upload pending and ignores late approval', async () => {
  ...
  assert.equal(result.error, '上传超时（5秒），请手动重试');
...
test('all text checks and the record write share a single five-second upload budget', async () => {
  const app = harness();
...
test('an earlier queue deadline limits both text review and the subsequent write', async () => {
  const app = harness();
...
test('an explicit later deadline cannot extend a single upload beyond five seconds', async () => {
  const app = harness();
...
for (const experience of [[], ['未审核心得']]) {
  test(`an expired queue budget sends no request (${JSON.stringify(experience)})`, async () => {
    const app = harness();
...
test('a hung upload and independent cloud refresh release locks for an explicit manual retry', async () => {
  ...
  await app.advance(5000);
  assert.equal(app.requests.length, 1, 'manual upload must finish without adding a cloud refresh wait');
  ...
  assert.deepEqual(app.requests[1].data, app.requests[0].data, 'retry must use the saved idempotent payload');
  app.requests[1].success({ result: { success: true, data: { recordId: 'saved-after-retry' } } });
  const result = await retry;
  assert.equal(result.success, true);
  assert.equal(result.refreshed, false);
  assert.equal(result.pending, 0);
  const firstRefresh = manager.refreshFromCloud();
  assert.equal(app.requests[2].data.type, 'getAllRecords');
  await app.advance(5000);
  assert.equal(await firstRefresh, false);
  const nextRefresh = manager.refreshFromCloud();
  app.requests[3].success({ result: { success: true, data: [{ ...record, _id: 'saved-after-retry' }] } });
  assert.equal(await nextRefresh, true);
  app.requests[0].success({ result: { success: true, data: { recordId: 'late-first-response' } } });
  app.requests[2].success({ result: { success: true, data: [] } });
  await flush();
